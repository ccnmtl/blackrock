...
class Species:
    ...
    @classmethod
    def get(cls, key, default, fmt='string'):
...
def calculate(request):
    ...
    try:
        ...
        msg = json.dumps({"error": "Plot size and/or plot count is too big"})
    ...
    intermed_dbh += sub['dbh'] * sub['count']  # weighted sum, used for average
    ...
    # sample variance dbh ##
    results['sample-variance-dbh'] = round2(variance(
        dbh_list, results['sample-dbh'], results['sample-count'] - 1))

    # sample variance density ##
    results['sample-variance-density'] = round2(variance(
        density_list, results['sample-density'], num_plots - 1))

    # sample variance basal area ##
    results['sample-variance-basal'] = round2(variance(basal_list,
...
class RandomSample:
    ...
    def circlePoints(self, point):
        ...
        return [(x_deg + rad * math.cos(angle),
                 y_deg + rad * math.sin(angle))
                for angle in
    ...
    def travel_time(self, point):
        # travel: 3 minutes per 100m (TODO: from NE corner? from prev plot?)
...
def sample_plot(sample, point, p_index):
    ...
    # unique species ##
    species_list_intermed = sets.Set([str(tree.species).lower()
                                      for tree in trees])
    ...
    # dbh variance ##
    results['variance-dbh'] = round2(variance(
        dbhs, results['dbh'], results['count'] - 1))
    ...
    results['time-total'] = (
        results['time-travel'] +
        results['time-locate'] +
        results['time-establish'] +
        results['time-measure'])
        ...
        if tree_count == results['count']:
            ...
            species_totals[i] = {
                ...
                'basal': round2(
                    (float(dbh_sum) * 0.785398) / float(results['area'])),
                'variance-dbh': round2(
                    variance(dbhs, mean_dbh, tree_count - 1))
            }
        i += 1
...
def trees_csv(request):
    ...
    if request.POST['results'] == '':
        return response
    writer = csv.writer(response)
    writer.writerow(['Sample Run', 'Plot',
...
def export_csv(request):
    ...
    results = json.loads(request.POST['results'])
    sample_num = re.split('\W', request.POST.get('sample_num', '0'), 1)[0]
    export_type = re.split('\W', export_type, 1)[0]

    response = HttpResponse(mimetype='text/csv')
    response['Content-Disposition'] = \
        'attachment; filename=%s_%s.csv' % (export_type, sample_num)
    writer = csv.writer(response)

    # write summary results
    writer.writerow(['**SAMPLING SUMMARY AND ANALYSIS**'])
    writer.writerow(['TIME TO SAMPLE', 'AVERAGE TIME PER PLOT',
                     'SAMPLE VARIANCE DENSITY', 'SAMPLE VARIANCE BASAL AREA'])
    writer.writerow([results['sample-time'],
                     results['avg-time'],
                     results['sample-variance-density'],
                     results['sample-variance-basal']])
    writer.writerow([])
    writer.writerow(['', 'AREA', 'SPECIES', '# OF TREES', 'MEAN DBH',
                     'VARIANCE DBH', 'DENSITY', 'BASAL AREA'])
    writer.writerow(['SAMPLE PLOTS', results['sample-area'],
                     results['sample-species'], results['sample-count'],
                     results['sample-dbh'], results['sample-variance-dbh'],
                     results['sample-density'], results['sample-basal']])
    writer.writerow(['FOREST POPULATION', results['actual-area'],
    ...
    writer.writerow(['COMPARISON', results['comparison-area'],
                     results['comparison-species'],
                     results['comparison-count'],
                     results['comparison-dbh'],
                     results['comparison-variance-dbh'],
                     results['comparison-density'],
    ...
    writer.writerow(['PLOT NUMBER', 'PLOT AREA',
                     'TREES', 'TOTAL SAMPLING TIME'])
    plots = results['plots']

    for plot, plotinfo in enumerate(plots):
        row = [int(plot) + 1, plotinfo['area'],
               plotinfo['count'], plotinfo['time-total']]
...
def loadcsv(request):
    if request.method == 'POST':
        ...
        for row in table:
            ...
            Tree.objects.get_or_create(id=row_id, location=loc,
                                       species=species, dbh=dbh, plot=p)
...
def test(request):
    ...
    return render_to_response("optimization/test.html",
                              {'trees': trees})
