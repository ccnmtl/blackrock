...
# TODO: replace these constants with distances calculated using the
# Haversine formula.
...
def walk_transect(start_point, meters, heading):
    """ Returns the lat and long of where you get after walking x meters
    from a starting point following a certain heading. Heading is assumed
    to be in radians clockwise from north."""
...
def to_heading(angle_in_radians):
    """converts to counterclockwise from east (trig standard)
    to clockwise from north (map standard)"""
...
def convert_from_heading_to_trig_radians(angle_in_degrees):
    """converts from clockwise from north (map standard)
    to counterclockwise from east (trig standard) """
...
def length_of_transect(heading_in_radians, side_of_square):
    """Returns the length of a line connecting the center
...
def circular_mean(angles):
    """ The average angle. If the angles are all pointing north,
    the average angle will be north."""
    x = y = 0.
    for angle in angles:
        x += cos(angle)
        y += sin(angle)
...
def smallest_difference(angles):
    ...
    all_angle_differences = [
        angle_difference(angles[m], angles[n]) for (m, n) in pairs]
...
def pick_transect_angles(number_needed):
    """Pick a bunch of transect angles at random,
    and pick the ones that are the most useful.
    The rules are:
    ...
    for a in range(number_of_tries):
        transects = sorted(pick_transect_heading()
                           for n in range(number_needed))
        sd = smallest_difference(transects)
        results.append(
            {
                'sd': sd, 'transects': transects
            }
        )
...
def pick_new_distance(existing_distances, transect_length, minimum_distance):
    ...
    *and* not too close to an existing distance"""
    ...
    while (min([abs(d - new_distance)
                for d in existing_distances]) < minimum_distance):
        new_distance = triangular_random(transect_length)
        if safeguard >= 10:
            return new_distance
...
def minimum_distance_between_points(transect_length,
                                    number_of_points_per_transect):
    """ A reasonable minimum distance separating points
    chosen at random along a line."""
    factor = 1.0 / 3.0
...
def rotate_about_a_point(point, center, angle_to_rotate):
    ...
    return to_lat_long(center_y + cos(new_angle) * r,
                       center_x + sin(new_angle) * r)
