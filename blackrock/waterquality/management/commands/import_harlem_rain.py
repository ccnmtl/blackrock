...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        reader = csv.reader(
            open("waterquality/xls/HarlemRiver_rain_ambtemp.csv"))
        ...
        (series, created) = Series.objects.get_or_create(
            name="Harlem Daily Rainfall", location=location, units=units)

        if not created:
            # it already exists,
            # so let's clear out all the data in that column
        ...
        for row in reader:
            ...
            for hour in range(24):
                ...
                Row.objects.create(series=series, timestamp=datetime,
                                   value=str(daily_rainfall * 25.4))
