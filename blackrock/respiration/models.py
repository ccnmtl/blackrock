...
class Temperature(models.Model):
    ...
    def arrhenius_sum(self, e0, r0, t0, delta_t, start_date, end_date,
        ...
        if use_python:
            try:
                goodyear = Temperature.objects.filter(
                    date__gte=start_date, date__lt=end_date,
                    precalc__isnull=False, station=station)
                for r in goodyear:
                    summation += math.exp(
                        e0 * (1 / t0 - 1 / (r.reading + 273.15)) / Rg)
            except:
                pass
            ...
            sqlcmd = """SELECT sum(exp(%f*(1/%f - 1/("%s"+273.15+%f)) /%f))
