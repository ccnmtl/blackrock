...
function display_next_specimen() {
    ...
    if (!nextElem) {
        ...
        replaceChildNodes("pollen-image", IMG({
            'src': STATIC_URL + 'images/paleoecology/pollen/' + images[current][0]
        }, null));
        $("pollen-image-large").src = STATIC_URL + 'images/paleoecology/pollen/' + images[current][2];
...
function setup_zoo() {
    ...
    for (var i = 0; i < images.length; i++) {
        var img = IMG({
            'id': 'image' + i,
            'src': STATIC_URL + 'images/paleoecology/pollen/' + images[i][0]
        }, null);
        ...
        appendChildNodes(zoo, DIV({
            'id': 'pollen-zoo-image' + i,
            'class': 'pollen-zoo-image unanswered'
        }, img, namediv, answerdiv));
    }
