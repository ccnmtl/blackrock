...
class Command(BaseCommand):
    def handle(self, *app_labels, **options):
        ...
        new_expeditions()
        new_points = create_points (clean_data)
        #teardown()
...
def clean_up (data):
    for id, values in data.iteritems():
        for k in ['bait', 'animal', 'habitat', 'understory', 'square_number', 'student_name', 'team_name', 'bait_left']:
...
def create_points (data):
    for id, values in data.iteritems():
        ...
        if exp.grid_square_id == None:
            square_coords = values ['square_number']
            exp.grid_square_id = [g.id for g in GridSquare.objects.all() if g.battleship_coords() == square_coords][0]
        ...
        #geography:
        #TODO some duplicates inside these model methods here.. can be removed.
        new_point.set_transect_bearing_wrt_magnetic_north (values ['bearing'])
        new_point.set_actual_lat_long([values ['lat'], values ['lon']])
        new_point.set_suggested_lat_lon_from_mag_north (values ['bearing'], values['distance'])
