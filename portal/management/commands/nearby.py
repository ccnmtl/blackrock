...
class Command(BaseCommand):
  ...
  def handle(self, *app_labels, **options):
      ...
      qs = Location.objects.filter(latlong__distance_lte=(loc.latlong,D(mi=.1))) 
      
      for loc in qs:
          all_related = loc._meta.get_all_related_objects()
          for obj in all_related:
               for instance in getattr(loc, obj.get_accessor_name()).all():
                   print "%s, %s, [%s, %s]" % (instance._meta.object_name, instance.name, instance.location.latlong.x, instance.location.latlong.y)
