...
class Command(BaseCommand):
    ...
    def prepare_database(self, app_name):
      ...
      models = get_models(module) # Assumes models are in dependency order -- todo: anyway to do this without hard-coding?
      models.reverse()
      for m in models:
    ...
    def process_model_object(self, entry, model, app_name):  
      try:
        ...
        print "Unexpected error: %s-%s" % (app_name, model._meta.verbose_name), sys.exc_info()
    ...
    def handle(self, *app_labels, **options):
        args = 'Usage: python manage.py import --user user --pwd password --sheet model spreadsheet name --app django app name]'
        
        if len(options) < 4:
          print args
          return
        elif (not self.init_google_client(options.get('user'), options.get('pwd'))):
            print args
            return
        else:
