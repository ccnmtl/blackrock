...
def import_csv(request):
  if request.method == 'POST':
    ...
    for row in table:
       ...
       tree = Tree.objects.get_or_create(id=id, location=loc, species=species, dbh=dbh)

    return HttpResponseRedirect(request.build_absolute_uri("../admin/sampler/tree"))

def index(request):
  return render_to_response('sampler/index.html');

def plot(request):
  # handle form data if present (will happen if doing "back" from later forms)
  arglist = {}
  try:
...
def transect(request):
  ...
  #print "%s, %s, %s, %s, %s, %s" % (x_offset, y_offset, scale, view_height, view_width, plot_h)
  ...
  visible_y_min = (float(plot_h) - float(view_height) + float(y_offset)) / float(scale) - 1
  visible_y_max = ((float(plot_h) + float(y_offset))) / float(scale) + 1

  valid_locations = Location.objects.filter(x__range=(str(visible_x_min),str(visible_x_max)),
  ...
  for tree in tree_list:
    ...
    new_y = float(tree.location.y) - (float(plot_h) - float(view_height) + float(y_offset)) / float(scale)
...
def worksheet(request):
  ...
  visible_y_min = (float(plot_h) - float(view_height) + float(y_offset)) / float(scale)
  ...
  ylocs = {}
  for tree in tree_list:
    new_x = math.cos(theta) * (float(tree.location.x) - x1) + math.sin(theta) * (float(tree.location.y) - y1) + x1
    new_y = -1 * math.sin(theta) * (float(tree.location.x) - x1) + math.cos(theta) * (float(tree.location.y) - y1) + y1
    #print "moving %s (%s,%s) to (%s,%s)" % (tree.id, tree.location.x, tree.location.y, new_x, new_y)
    new_x = new_x + float(x_offset) / float(scale)
    new_y = new_y - (float(plot_h) - float(view_height) + float(y_offset)) / float(scale)
...
def export_csv(request):
  ...
  response['Content-Disposition'] = 'attachment; filename=sampler-worksheet.csv'
