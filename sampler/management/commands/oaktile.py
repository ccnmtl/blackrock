...
# Calculate the global average dbh for each species set and use this number to populate missing dbh in the code

# Missing data instructions -- 
# * Missing DBH -- Fill in the blanks using a global average for a given species set (oak, non-oak)
...
class Command(BaseCommand):
    ...
    avg_oak_dbh = 43.9
    avg_nonoak_dbh = 20  
    centered_data_plots = ["A4", "B1", "B2", "B3", "B4", "C1", "C2", "C3", "C4"]
    plot_origin_xy = {"A1":[0,0], "A2":[75,0], "A3":[150,0], "A4":[225,0], "B1": [0,75], "B2": [75,75], "B3":[150,75], "B4":[225,75], "C1":[0,150], "C2":[75,150], "C3":[150,150], "C4":[225,150] }
        
    idx = {}
    tree_id_counter = 1
        
    option_list = BaseCommand.option_list + (
        make_option('--csv', dest='csv', help='Base CSV file containing oak and non-oak data. non-oak data will be tiled.'),
    )
    ...
    def process_non_oak(self, plot, id, x, y, species, dbh):
        ...
        # for plots with a centered_data_plot, transform points to upper left hand corner
        offsetx = x
        offsety = y
        if (plot in self.centered_data_plots):
            # subtract 25,25 from the x/y coordinates to transform the coordinates to the upper left hand corner
            offsetx -= 25
            offsety -= 25
        
        origin_xy = self.plot_origin_xy[plot]
        if (offsetx < origin_xy[0] or offsetx > origin_xy[0]+25 or offsety < origin_xy[1] or offsety > origin_xy[1]+25):
            sys.stderr.write("%s %s Tree Location (%s,%s) >> (%s, %s) is outside the 25x25 plot. Removing tree from tiling\n" % (plot, id, x, y, offsetx, offsety))
            return
        ...
        for tiles_y in range(3):
            for tiles_x in range(3):    
                ...
                print '%s,%s,%s,%s,%s' % (self.tree_id_counter, species, newx, newy, dbh)
    ...
    def handle(self, *app_labels, **options):
        ...
        for tree in trees:
            ...
            if (species in self.oaks):
                ...
                print '%s,%s,%s,%s,%s' % (self.tree_id_counter, species, x, y, dbh)
                self.tree_id_counter += 1
            else:
                 # If species is missing, assumes non-oak
                self.process_non_oak(plot, id, float(x), float(y), species, dbh)
