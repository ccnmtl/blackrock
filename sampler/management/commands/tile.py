...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('--plotsize', dest='plotsize', help='Original plot size. Assuming square dimensions'),
        make_option('--factor', dest='factor', help='The factor by which to multiply the plot size'),
    )
            
    def handle(self, *app_labels, **options):
        from django.db.models import get_app, get_apps, get_models
        
        args = 'Usage: python manage.py tile --csv csv file --plotsize original size --factor multiplication factor]'
        
        if (not options.get('csv') or not options.get('plotsize') or not options.get('factor')):
            print args
            return
