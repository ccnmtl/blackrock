...
function moveSlider() {
  ...
  setElementPosition("windowdrag", {'y':Math.round(coreHeight * ratio)});
}
...
function setupSlider() {
  ...
  zoomedHeight = $("core-zoomed").scrollHeight - parseInt(getStyle("core-zoomed", 'height'));
  
  var draggerHt = getElementDimensions("windowdrag").h;
  coreHeight = getElementDimensions("core-unzoomed").h - draggerHt;
  
  new Draggable('windowdrag', {snap: function(x,y) {
    var newy = y;
    if (y < 0) {
      newy = 0;
    }
